...
class SellerProductListViewModelTest {
    ...
    fun `load should read seller products from backend cafe`() = runTest {
        ...
        every { getFoodsByCafeUseCase.invoke("cafe-1", 0, 20, "", null, null, null) } returns flowOf(
            Resource.Success(
                PagedData(
                    content = listOf(
                        Food(
                            ...
                            images = listOf("https://media.shopme.test/products/coffee/medium.jpg"),
                            variants = emptyList()
                        )
                    ),
                    page = 0,
        ...
        assertEquals("https://media.shopme.test/products/coffee/medium.jpg", vm.uiState.value.products.first().image)
    }
    ...
    fun `search and filters should reload first page with selected params`() = runTest {
        ...
        every { getFoodsByCafeUseCase.invoke("cafe-1", 0, 20, "coffee", null, null, null) } returns flowOf(
            Resource.Success(PagedData(content = emptyList(), page = 0, last = true))
        )
        every { getFoodsByCafeUseCase.invoke("cafe-1", 0, 20, "coffee", FoodCategory.DRINK, null, null) } returns flowOf(
            Resource.Success(PagedData(content = emptyList(), page = 0, last = true))
        )
        ...
        vm.onEvent(SellerProductListEvent.Load)
        advanceUntilIdle()
        vm.onEvent(SellerProductListEvent.ChangeSearchQuery("coffee"))
        advanceUntilIdle()
        vm.onEvent(SellerProductListEvent.SelectCategoryFilter(FoodCategory.DRINK))
        advanceUntilIdle()
        vm.onEvent(SellerProductListEvent.SelectStatusFilter(FoodStatus.READY))
        advanceUntilIdle()

        assertEquals("coffee", vm.uiState.value.searchQuery)
        assertEquals(FoodCategory.DRINK, vm.uiState.value.categoryFilter)
        assertEquals(FoodStatus.READY, vm.uiState.value.statusFilter)
    }
}
