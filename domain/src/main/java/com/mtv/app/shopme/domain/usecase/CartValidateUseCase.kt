/*
 * Project: Shopme App
 * Author: Boys.mtv@gmail.com
 * File: CartOrderUseCase.kt
 *
 * Last modified by Dedy Wijaya on 03/03/26 13.35
 */

package com.mtv.app.shopme.domain.usecase

import com.mtv.app.core.provider.based.BaseUseCase
import com.mtv.app.shopme.data.remote.api.ApiEndPoint
import com.mtv.app.shopme.data.remote.api.ApiResponse
import com.mtv.app.shopme.data.remote.request.CartValidateRequest
import com.mtv.app.shopme.data.remote.response.CartValidateResponse
import com.mtv.based.core.network.di.IoDispatcher
import com.mtv.based.core.network.repository.NetworkRepository
import javax.inject.Inject
import kotlinx.coroutines.CoroutineDispatcher

class CartValidateUseCase @Inject constructor(
    private val repository: NetworkRepository,
    @IoDispatcher dispatcher: CoroutineDispatcher
) : BaseUseCase<CartValidateRequest, ApiResponse<CartValidateResponse>>(dispatcher) {

    override suspend fun execute(param: CartValidateRequest) = repository.request<ApiResponse<CartValidateResponse>>(
        endpoint = ApiEndPoint.PostValidateCart,
    )

}