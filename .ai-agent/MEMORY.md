...
- Local generated release keystore is intentionally git-ignored at `keystore/shopme-release.jks`; base64 helper output is `keystore/KEYSTORE_BASE64.txt`. Current release key alias is `shopme`; SHA-256 fingerprint is `[REDACTED]`.
...
- 2026-05-17 21:16: Android performance/chat lifecycle pass completed for latest user notes. Chat detail now has a Room-backed `chat_message_cache` so customer/seller chat screens can show cached messages first and refresh remote data instead of making every open feel like a full heavy fetch. Realtime websocket usage now has `retain()/release()` lifecycle ownership in chat, notification, order, seller dashboard/order/product-related realtime screens; ViewModels retain once on load and release in `onCleared`, and the singleton gateway closes the socket when no realtime screen remains active. Profile/address writes now refresh the cached customer after add/delete/default-address changes so add-to-cart/session checks do not keep reading stale incomplete address data. Chat message mapping now uses backend `messageId` for unique local/cache row identity. Full Android `.\gradlew.bat testDebugUnitTest assembleDebug` passed after updating realtime lifecycle tests. Commit `f6e8ed1` was pushed to `master`. Fastlane Firebase App Distribution upload to `internal-tester` succeeded with notes `Shopme debug build - chat cache realtime socket lifecycle address cache refresh`; console `https://console.firebase.google.com/project/shopme-app-360b4/appdistribution/app/android:com.mtv.app.shopme/releases/38b4237aev8v8?utm_source=fastlane`, tester `https://appdistribution.firebase.google.com/testerapps/1:870455241065:android:76c3f0351cddb70a5f75f8/releases/38b4237aev8v8?utm_source=fastlane`.
- 2026-05-17 21:47: Android chat detail paged API consumption completed and uploaded. Customer and seller chat detail now load initial messages through backend `GET /api/chat/page` using a latest-page probe, emit cached Room messages first, and load older pages automatically when the user scrolls to the top instead of fetching the entire conversation. Added domain/repository/use-case support for paged chat messages, buyer/seller ViewModel pagination state, top inline loading indicators, and updated buyer/seller chat tests to use `PagedData`. Verification passed: targeted Kotlin compile for data/domain/customer/seller, focused customer/seller unit tests, full `.\gradlew.bat testDebugUnitTest assembleDebug`, and Android/backend contract verifier with `--skip-runtime`. Commit `4c16185` was pushed to `master`. Fastlane Firebase App Distribution upload to group `internal-tester` succeeded with notes `Shopme debug build - paged chat detail API with cached first load`; console `https://console.firebase.google.com/project/shopme-app-360b4/appdistribution/app/android:com.mtv.app.shopme/releases/2p4n3qt9bq39o?utm_source=fastlane`, tester `https://appdistribution.firebase.google.com/testerapps/1:870455241065:android:76c3f0351cddb70a5f75f8/releases/2p4n3qt9bq39o?utm_source=fastlane`.
- 2026-05-17 22:50: Android seller flow audit/fix completed and uploaded. Seller order list now sends the selected status filter to backend `GET /api/seller/orders/page`, so filtered tabs no longer depend on only the already-loaded client pages. Seller dashboard now loads only the first paged order summary batch instead of the legacy all-orders endpoint, seller product search reload is debounced by 350 ms, and seller profile/home routing now returns to seller dashboard instead of old customer-home behavior. Verification passed: targeted Android compile for data/domain/feature-seller/nav, `:feature-seller:testDebugUnitTest`, full `.\gradlew.bat testDebugUnitTest assembleDebug`, backend targeted tests, full backend `.\mvnw.cmd test`, Docker backend rebuild, live Android/backend contract verifier with `--skip-runtime`, and runtime seller smoke showing `GET /api/seller/orders/page?status=ORDERED` returns only `ORDERED` rows. Fastlane Firebase App Distribution upload to group `internal-tester` succeeded with notes `Shopme debug build - lighter seller order filters dashboard search and profile routing`; console `https://console.firebase.google.com/project/shopme-app-360b4/appdistribution/app/android:com.mtv.app.shopme/releases/2uitnh89n6h58?utm_source=fastlane`, tester `https://appdistribution.firebase.google.com/testerapps/1:870455241065:android:76c3f0351cddb70a5f75f8/releases/2uitnh89n6h58?utm_source=fastlane`.
- 2026-05-18 12:52: Android list-performance continuation completed and uploaded. Seller product management now consumes backend aggregate stats from `GET /api/foods/cafe/{cafeId}/stats` for total products/stock, while customer orders consume paged `GET /api/order/page` with cache-first page 0 fallback and scroll-triggered load-more. Heavy list screens were tightened with stable LazyColumn/LazyVerticalGrid keys for home, customer/seller chat lists, customer/seller notifications, customer orders, seller orders, and seller products; seller order/product pagination is now driven by scroll state instead of composition-time load-more calls. Verification passed before commit/upload: targeted customer+seller unit tests, targeted data/domain/customer/seller compile with KSP incremental disabled, full Android `.\gradlew.bat testDebugUnitTest assembleDebug "-Pksp.incremental=false" --no-daemon`, Android/backend static contract verifier against live local OpenAPI with `--skip-runtime`, and runtime smoke for customer order page (`orderPageCount=5`, `last=false`) plus seller product stats (`totalProducts=54`, `totalStock=2061`). Commits pushed: `d4f5aca feat: consume seller product stats`, `05a93f1 perf: page customer order list`, and `9e33716 perf: stabilize list rendering keys`. Fastlane Firebase App Distribution upload to group `internal-tester` succeeded with notes `Shopme debug build - lighter list rendering paged orders and seller product stats`; console `https://console.firebase.google.com/project/shopme-app-360b4/appdistribution/app/android:com.mtv.app.shopme/releases/05i7bpkj5pnkg?utm_source=fastlane`, tester `https://appdistribution.firebase.google.com/testerapps/1:870455241065:android:76c3f0351cddb70a5f75f8/releases/05i7bpkj5pnkg?utm_source=fastlane`.
